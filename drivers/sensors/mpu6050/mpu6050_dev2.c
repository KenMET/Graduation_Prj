...
struct mpu6050_device *mpu6050_dev2;
...
static long mpu_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    switch(cmd) {  
    	case 2:
			mpu_ev.q1 = mpu6050_dev2->mpu_ev->q1;
			mpu_ev.q2 = mpu6050_dev2->mpu_ev->q2;
			mpu_ev.q3 = mpu6050_dev2->mpu_ev->q3;
			mpu_ev.q4 = mpu6050_dev2->mpu_ev->q4;
			break;
		default:
...
static struct file_operations mpu6050_dev2_fops = {
    .owner			= THIS_MODULE,
    .open			= mpu_open,
    .release		= mpu_release,
...
static int mpu_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	cdev_init(&mpu->cdev, &mpu6050_dev2_fops);  
	...
	snprintf(mpu->phys, sizeof(mpu->phys),
		 "%s/mpu6050_dev2", dev_name(&client->dev));
...
static const struct i2c_device_id mpu6050_dev2_id[] = {
	{MPU_NAME, 0},
	{}
};

static struct i2c_driver mpu6050_dev2_driver = {
	.class = I2C_CLASS_HWMON,
	.probe = mpu_probe,
	.remove = mpu_remove,
	.id_table = mpu6050_dev2_id,
	.driver = {
		.owner = THIS_MODULE,
		.name = MPU_NAME,
	},
...
{
	int flag = i2c_add_driver(&mpu6050_dev2_driver);
	mpu_log("mpu6050_dev2 driver init1!flag:%d\n", flag);
