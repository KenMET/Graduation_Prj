...
#elif defined(CONFIG_FB_S5P_DUMMY_MIPI_LCD) || defined(CONFIG_FB_S5P_ZM80076A)
#define		LCD_BUS_NUM	3
#define		DISPLAY_CS	EXYNOS4_GPB(5)
#define		DISPLAY_CLK	EXYNOS4_GPB(4)
#define		DISPLAY_SI	EXYNOS4_GPB(7)
...
static int reset_lcd(void)
{
	...
	printk("SCP(%s, %d): TP1_EN_ON\n", __FUNCTION__, __LINE__);
	mdelay(50);

	//RESET
/* add by cym 20131114 */
#ifdef CONFIG_FB_S5P_ZM80076A
	err = gpio_request(EXYNOS4_GPB(4), "GPB_4");
	if (err) {
		printk(KERN_ERR "failed to request GPB_4 for "
			"MIPI reset\n");
		return err;
	}
	...
	if (err) {
		printk(KERN_ERR "failed to request GPL0_3 for "
			"MIPI reset\n");
		return err;
	}
	//gpio_direction_output(EXYNOS4_GPL0(3), 1);
	...
	s3c_gpio_cfgpin(EXYNOS4_GPL0(3), S3C_GPIO_OUTPUT);
	gpio_free(EXYNOS4_GPL0(3));
#endif

	printk("SCP(%s, %d): MIPI Reset\n", __FUNCTION__, __LINE__);
	mdelay(50);
#endif		
...
#if  defined(CONFIG_CPU_TYPE_SCP_SUPPER) || defined(CONFIG_CPU_TYPE_SCP_SUPPER) || defined(CONFIG_CPU_TYPE_POP2G_SUPPER)
